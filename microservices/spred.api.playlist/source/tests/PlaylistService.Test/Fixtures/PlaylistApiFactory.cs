...
namespace PlaylistService.Test.Fixtures;
...
public class PlaylistApiFactory : WebApplicationFactory<Program>
{
    ...
    public Mock<IPersistenceStore<CatalogMetadata, Guid>> CatalogDataMock { get; } = new();
    public Mock<IPersistenceStore<CatalogStatistics, Guid>> CatalogStatistics { get; } = new();
    ...
    {
        var testObj = new TestCatalogMetadata(){ };
        testObj.SetId(Guid.Empty);
        
        SetupPersistenceStoreMock<CatalogStatistics, Guid, long>(CatalogStatistics, () => new CatalogStatistics()
            {
                Followers = 0,
                FollowersDailyDiff = 12,
                Date = DateTime.Now
            }
        );
        SetUpRedisDatabase();
        ...
        builder.ConfigureServices(services =>
        {
            ...
            if (EnableTestAuth)
            {
                ...
                services.AddAuthorization(options =>
                {
                    ...
                    options.AddPolicy(JwtSpredPolicy.JwtUserPolicy, policy =>
                        policy.RequireAssertion(_ => true));
                    options.AddPolicy(Policies.PlaylistCreate, policy =>
                        policy.RequireAssertion(_ => true));
                    options.AddPolicy(Policies.PlaylistDeleteOwn, policy =>
                        policy.RequireAssertion(_ => true));
                    options.AddPolicy(Policies.PlaylistEditOwn, policy =>
                        policy.RequireAssertion(_ => true));
                    options.AddPolicy(Policies.PlaylistOwnPrivateRead, policy =>
                        policy.RequireAssertion(_ => true));
            ...
            services.RemoveAll<IPersistenceStore<CatalogMetadata, Guid>>();
            services.AddScoped<IPersistenceStore<CatalogMetadata, Guid>>(_ => CatalogDataMock.Object);
            
            services.RemoveAll<IPersistenceStore<CatalogStatistics, Guid>>();
            services.AddScoped<IPersistenceStore<CatalogStatistics, Guid>>(_ => CatalogStatistics.Object);
        });
