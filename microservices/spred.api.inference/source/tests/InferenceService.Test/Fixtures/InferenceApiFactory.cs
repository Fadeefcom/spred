...
using InferenceService.Test.Helpers;
...
namespace InferenceService.Test.Fixtures;

public class InferenceApiFactory: WebApplicationFactory<Program>
{
    ...
    public Mock<IPersistenceStore<InferenceResult, Guid>> DataMock { get; } = new();
    ...
    {
        SetUpRedisDatabase();
        SetupPersistenceStoreMock<InferenceResult, Guid, long>(DataMock, () => new InferenceResult()
        {
            Metadata =
            {
                new InferenceMetadata()
                {
                    MetadataId = Guid.NewGuid(),
                    MetadataOwner = Guid.Empty,
                ...
                new InferenceMetadata()
                {
                    ...
                    Reaction = new ReactionStatus(),
                    Score = 0.2f,
                    SimilarTracks = new List<SimilarTrack>()
                    {
                        new SimilarTrack()
                        {
                            SimilarTrackId = Guid.NewGuid(),
                            TrackOwner = Guid.Empty,
                            Similarity = 0.8f
                        }
                    },
                    Type = "record_label"
        ...
        builder.ConfigureServices(services =>
        {
            ...
            services.RemoveAll<IPersistenceStore<InferenceResult, Guid>>();
            services.AddScoped<IPersistenceStore<InferenceResult, Guid>>(_ => DataMock.Object);
        });
